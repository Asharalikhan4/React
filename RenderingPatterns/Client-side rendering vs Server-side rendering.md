...
- Even though the [capabilities of JavaScript](https://learnersbucket.com/tutorials/frontend-system-design/where-can-we-use-javascript/) have increased and it can now be used to create mobile apps and desktop apps, but its prominent usage is still more for the web applications.
...
![Related Image](../public/images/MPA-SSR1.webp)
...
- Best suited for applications that require search engine optimization. (Google Bots, Search Engine Crawlers and Social Media Bots, LCP)
...
![Related Image](../public/images/SPA-CSR1.webp)
- To understand why single page application is effective, we should first understand [how an HTML page is rendered in the browser](https://learnersbucket.com/examples/web/how-a-web-page-is-rendered-in-browser/).
...
- The modern JavaScript frameworks are trying to solve this by minimizing the DOM manipulation and HTML rendering with the implementation of [Virtual DOM](https://www.syncfusion.com/blogs/post/react-virtual-dom) and other better reconciliation algorithms to update only what changes.
...
- **Blazing fast**, as the static files are loaded only once on the initial render, and after that only required data is pulled from the server and UI is updated on runtime. Also the the [JavaScript code that is not required on initial load can be split in different bundles and lazy loaded](https://learnersbucket.com/examples/web/load-websites-faster-with-code-splitting/), reducing the JavaScript execution time in browser.
